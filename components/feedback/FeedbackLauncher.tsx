...
export default function FeedbackLauncher() {
  ...
  // Update payload with user info when authenticated
  useEffect(() => {
    if (user) {
      setPayload((p) => ({
        ...p,
        ...
        >
          ...
          <div className="relative w-full sm:max-w-lg sm:rounded-2xl sm:shadow-2xl bg-neutral-900 text-neutral-50 p-0 m-0 sm:m-4 overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-fuchsia-600 via-purple-600 to-indigo-600 px-5 py-4">
              ...
              {(() => {
                const initials = identity?.initials ?? user?.email?.[0] ?? "U";
                const userLabel = identity?.displayName || user?.email || "Signed-in user";
                return (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <label className="text-sm">
                  ...
                  <input
                    type="email"
                    inputMode="email"
                    placeholder={user ? user.email || "No email available" : "we'll reach out if needed"}
                    value={payload.email}
                    ...
                    className={`mt-1 w-full rounded-xl border px-3 py-2 text-sm
                               focus:outline-none focus:ring-2 focus:ring-white/20
                               ${user 
                                 ? "bg-neutral-700 border-white/5 text-neutral-300 cursor-not-allowed" 
