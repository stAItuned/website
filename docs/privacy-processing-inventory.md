...
|---|---|---|---|---|---|---|---|
| Article views first-party | `POST /api/analytics/page-view`, `GET /api/admin/analytics/pages`, `/admin/analytics` | article slug, timestamp, contatore aggregato, metadati contenuto (titolo/autore/locale/target) in sola lettura admin | metriche editoriali aggregate senza GA + ranking interno admin sui contatori first-party | legittimo interesse; vedi purpose split in `docs/privacy-lawful-basis-matrix.md` | Firebase/Firestore | contatore cumulativo (no profilo utente) | privacy page (blog visits section) + admin analytics doc |
