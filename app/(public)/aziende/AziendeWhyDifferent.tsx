...
/**
 * AziendeWhyDifferent - Uniqueness + Quality section for business page
 * Shows what makes us different: time-to-value, quality, AI+Product+UX
 */
export function AziendeWhyDifferent() {
    const pillars = [
        {
            icon: (
    ...
    return (
        <section className="space-y-8">
            ...
            <div className="text-center">
                <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-50 leading-tight">
            ...
            <div className="grid gap-4 md:grid-cols-3">
                {pillars.map((pillar) => {
                    ...
                    return (
                        <article
                            key={pillar.title}
                            className={`group relative rounded-2xl border-2 ${classes.border} ${classes.bg} p-5 sm:p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1`}
                        >
                            {/* Icon */}
                            <div className={`inline-flex p-2.5 rounded-xl ${classes.iconBg} ${classes.icon} mb-4`}>
                                {pillar.icon}
                            </div>
