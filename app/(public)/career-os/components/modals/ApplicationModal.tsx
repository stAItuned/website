...
export default function ApplicationModal({ locale }: { locale: CareerOSLocale }) {
    const { isAppModalOpen, closeAppModal, appModalData } = useCareerOS()
    const t = careerOSTranslations[locale].applicationModal
    ...
    const currentStepLabel = useMemo(() => `${t.stepLabel} ${step} / 4`, [step, t.stepLabel])

    const validateStep = (currentStep: number) => {
        if (currentStep === 1) {
            if (!formData.name.trim()) return t.errors.nameRequired
            if (!formData.email.trim()) return t.errors.emailRequired
            if (!formData.background) return t.errors.backgroundRequired
            const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
            if (!emailRegex.test(formData.email.trim())) return t.errors.invalidEmail
        }
        if (currentStep === 2) {
            if (!formData.roleTarget) return t.errors.roleRequired
            if (!formData.timeline) return t.errors.timelineRequired
        }
        if (currentStep === 3) {
            if (!formData.mainBlock) return t.errors.blockRequired
            if (!formData.applicationsLastMonth) return t.errors.applicationsRequired
        }
        if (currentStep === 4) {
            if (!formData.acceptedPrivacy) return t.errors.privacyRequired
        }
        return ''
    }
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        try {
            ...
            setStatus('error')
            setMessage(err instanceof Error ? err.message : t.errors.submitFailed)
        }
    }
            ...
            <div className="relative z-10 w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-900/10 dark:border-slate-800 dark:bg-[#0F1117]">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <h3 className="mt-2 text-xl font-bold text-[#1A1E3B] dark:text-white">{t.title}</h3>
                ...
                {status === 'success' ? (
                    <div className="mt-8 text-center">
                        ...
                        <h4 className="mt-4 text-lg font-bold text-[#1A1E3B] dark:text-white">{t.successTitle}</h4>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        {t.options.background.map(option => (
                        ...
                        {step === 2 && (
                            <div className="space-y-4">
                                <div>
                                    ...
                                    >
                                        <option value="">{t.fields.selectPlaceholder}</option>
                                        {t.options.roleTarget.map(option => (
                                            <option key={option} value={option}>
                                                {option}
                                            </option>
                                        ))}
                        ...
                        {step === 3 && (
                            <div className="space-y-4">
                                <div>
                                    ...
                                    >
                                        <option value="">{t.fields.selectPlaceholder}</option>
                                        {t.options.mainBlock.map(option => (
                                            <option key={option} value={option}>
                                                {option}
                                            </option>
                                        ))}
                                ...
                                <div>
                                    <label className="text-xs font-semibold uppercase tracking-wide text-slate-400" htmlFor="career-applications">
                                        {t.fields.applicationsLastMonth}
                                    </label>
                                    <select
                                    ...
                                    >
                                        <option value="">{t.fields.selectPlaceholder}</option>
                                        {t.options.applicationsLastMonth.map(option => (
                                            <option key={option} value={option}>
                                                {option}
                                            </option>
                                        ))}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-start gap-2 pt-2">
                                    ...
                                    <label htmlFor="privacy-app" className="text-xs text-slate-500 dark:text-slate-400">
                                        {t.fields.privacyLabelPrefix}{' '}
                                        <a href="/privacy" className="underline hover:text-amber-500" target="_blank">{t.fields.privacyPolicy}</a>{' '}
                                        {locale === 'en' ? 'and in the' : 'e nei'} <a href="/terms" className="underline hover:text-amber-500" target="_blank">{t.fields.terms}</a>
                                    </label>
                                </div>
                            </div>
                        )}
