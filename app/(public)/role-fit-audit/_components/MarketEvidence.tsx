...
function BoldImpactCard({
  stat,
  icon: Icon,
}: {
  stat: { value: string; metric: string; label: string; source: string; link: string }
      ...
      <div className="relative z-10 flex flex-col items-center">
        <div className="relative mb-3">
          <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#FFF272] to-[#F59E0B] flex items-center justify-center shadow-lg shadow-[#FFF272]/30 group-hover:animate-pulse">
            <Icon className="w-6 h-6 text-[#1A1E3B]" />
          </div>
        ...
        <div className="text-center mb-2">
          ...
          <span className="block text-lg font-bold text-[#FFF272] uppercase tracking-wider mt-1">{stat.metric}</span>
        </div>

        <div className="flex items-center gap-2 mb-4">
          <div className="w-6 h-0.5 bg-gradient-to-r from-transparent to-[#FFF272]/50" />
          <p className="text-sm text-slate-300 text-center font-medium">{stat.label}</p>
...
export function MarketEvidence({ locale }: { locale: RoleFitLocale }) {
  const t = roleFitAuditTranslations[locale].marketEvidence
  const icons = [TrendingUp, AlertTriangle, Users] as const

  return (
    <div className="w-full max-w-5xl mx-auto mb-10 px-4">
      <p className="text-center text-xs font-semibold uppercase tracking-widest text-slate-500 mb-6">📊 {t.sectionLabel}</p>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {t.stats.map((stat, i) => (
          <BoldImpactCard key={`${stat.source}-${i}`} stat={stat} icon={icons[i] ?? TrendingUp} />
        ))}
      </div>
    </div>
  )
