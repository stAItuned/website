...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!user) {
            return jsonNoStore({ isWriter: false, hasProfile: false, hasAgreement: false }, { status: 200 });
        }
        ...
        if (!user.uid) {
            return jsonNoStore({ isWriter: false, hasProfile: false, hasAgreement }, { status: 200 });
        }
