import { roleFitAuditTranslations } from '../lib/i18n/role-fit-audit-translations'

const PLACEHOLDER_PATTERNS = [/\[TODO\]/i, /TBD/i, /TODO/i, /^\s*\.{3}\s*$/]
...
export function verifyRoleFitI18nParity(): { ok: boolean; issues: string[] } {
  const issues: string[] = []

  const it = flatten(roleFitAuditTranslations.it)
  const en = flatten(roleFitAuditTranslations.en)
...
const isDirectExecution =
  ...
  process.argv[1]?.includes('verify-role-fit-i18n')

if (isDirectExecution) {
  const result = verifyRoleFitI18nParity()

  if (!result.ok) {
    console.error('[role-fit-i18n] parity check failed')
  ...
  console.log('[role-fit-i18n] parity check passed')
}
