...
import type { Config as ContentlayerConfig } from '@contentlayer/core'
...
const runContentlayerDev = async ({ configPath }: NextPluginOptions) => {
  ...
  await pipe(
    ...
    S.tapRight((config: ContentlayerConfig) =>
      config.source.options.disableImportAliasWarning ? T.sync(() => undefined) : core.validateTsconfig()
    ),
    S.chainSwitchMapEitherRight((config: ContentlayerConfig) =>
      core.generateDotpkgStream({ config, verbose: false, isDev: true })
    ),
    S.tapLeft((error) => T.log(errorToString(error))),
    S.tapRight(core.logGenerateInfo),
    S.runDrain,
    runMain,
  )
}
